...
class ZonaB: 
    def __init__(self, user,cine):
        self.cine=cine
        self.todo = Frame(user,  bg = "#FAE5D3") #Este es lo que contiene toda la zona 2
        self.todo.pack(fill=X)
        self.funciones = {"Venta":self.venta,
                         "Agregar Pelicula":self.agregarPelicula,
                         "Eliminar Pelicula":self.quitarPelicula, 
        ...
        self.titulo = Label(self.titulo_texto,font=('Berlin Sans FB', 32), bg="#FAE5D3",text = "¡Bienvenido a la ventana de usuario!")  #label del titulo
        self.titulo.pack(fill=X)

        self.texto = Label(self.titulo_texto,font=('Berlin Sans FB', 24 ), bg = "#FAE5D3",text = "Por favor selecciona una opción del menú para continuar")   #label del titulo
        self.texto.pack(fill=X)

        self.cuerpo = Frame(self.todo,width=2, height = 800, bg= "#FAE5D3") #este es el cuerpo, se inicializa vacio
        self.cuerpo.pack(fill=X)
        
    # Metodo para devolverse a la ventana inicial de usuario
    def cambiar(self):
        self.cuerpo.pack_forget()
        self.cuerpo = Frame(self.todo,width=800, height = 800, bg= "#FAE5D3")

        self.titulo.configure(text = "AutoCine")
        self.texto.configure(text = "Bienvenido al AutoCine, seleccione lo que desea hacer")
    ...
    def agregarPelicula(self):
        ...
        def addPeli(action):
            ...
            messagebox.showinfo(title="Información",message="¡Pelicula agregada con éxito!")     #Se muestra un messagebox al crear la pelicula
    ...
    def quitarPelicula(self):
        ...
        def removePeli(action):
            ...
            messagebox.showinfo(title="Información",message="¡Pelicula eliminada del Autocine con éxito!")   #Se muestra un messagebox al crear la pelicula
            ...
            def horariosala(action):        ##Aca se muestran los horarios disponibles de la sala escogida en la fecha escogida
                ...
                def peliscine(action):        ###Peliculas disponibles en el cine
                    ...
                    def creacionfinal(action):
                        ...
                        messagebox.showinfo(title="Información",message="¡Funcion agregada con éxito!")    #Se muestra un mensaje cuando se completó la transacción
