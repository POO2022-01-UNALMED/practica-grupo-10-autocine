...
def ventana(variable, window, cine):
    ...
    def vender(existente):
        ...
        def mostrar_sillas(funcion):
            ...
            nueva.pack_forget()
            label.pack_forget()
            label = Label(venta, text="Seleccione el puesto que desea")
            label.pack()
            nueva = Frame(venta)
            nueva.pack()
            ...
            def vender_boleto(numero):
                ...
                nueva.pack_forget()
                nueva = FieldFrame("Se ha vendido el ticket", [], "numero " + str(numero + 1), [], None, venta)
        ...
        def mostrar_funciones(funciones):
            ...
            def obtenerFuncion():
                ...
                try:
                    ...
                    raise NotIn                                # Se lo contrario se lanza la excepción.

                mostrar_sillas(cine.BuscadorFuncion(numero))   # Acá llamamos la función que nos muestra las sillas.

            nueva.button.bind('<ButtonRelease>', lambda x: obtenerFuncion()) # Se le añade la función al botón para obtener la funcion.
        ...
        if (cine.buscadorCliente(numero) == None):  #Si el cliente no se encuentra en la base de datos se crea el frame que recibe todos los datos del                                       
            ...
            def crearCliente():
                ...
                if (int(frame.getValue("Cedula referido")) != 0):#si el cliente no cuenta con ningún referido se verifica que los datos correspondan 
                    ...
                    cliente.referidos()                                                                 #y se le aplica su respectivo descuento por                                                                                #referido 
                    vender(False)
